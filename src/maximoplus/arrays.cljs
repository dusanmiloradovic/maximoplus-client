...
(defn ^int index-of [arr  val]
                                        ;u IE8 nema indexOf metoda za array
  (if (aget (.-prototype js/Array) "indexOf")
    (.indexOf arr val)
    (loop [i 0]
      (if-let [curr (aget arr i)]
        (if (= curr val)
          i
          (recur (inc i))
          )
